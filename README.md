...
## Pipelines
This section details how the pipeline system works. Each pipeline is executed by a worker. The worker periodically checks the database to see if new entities have arrived in a bucket. If they have, the worker processes them, and writes them to another bucket. The pipelines uses the databae to keep track of where it is in the bucket. 

Pipelines are configured by a YAML file. 